...
void VoiceRecognizer::reload() {
	...
	if (_ps_config != nullptr) cmd_ln_free_r(_ps_config);
	...
	// Initialize config
	_ps_config = cmd_ln_init(nullptr, ps_args(), TRUE,
	"-hmm", MODELDIR "/en-us/en-us",
	"-lm", MODELDIR "/en-us/en-us.lm.bin",
	"-dict", dictionary_path.c_str(),
	"-logfn", "/dev/null",
	NULL);
	if (_ps_config == nullptr) {
...
// Destructor
VoiceRecognizer::~VoiceRecognizer() {
	if (_pulse != nullptr) pa_simple_free(_pulse);
	if (_ps != nullptr) ps_free(_ps);
	if (_ps_config != nullptr) cmd_ln_free_r(_ps_config);
}
...
bool VoiceRecognizer::wait_for_keyword() {
	...
	while (!(!_in_speech && _utt_started)) {
		...
		pa_simple_read(_pulse, _buf, 2048, nullptr);
...
int VoiceRecognizer::process_microphone(bool process) {
	if (process) {
		...
		if (config["use_pulseaudio_flush"] != NULL && config["use_pulseaudio_flush"])
			pa_simple_flush(_pulse, nullptr);
